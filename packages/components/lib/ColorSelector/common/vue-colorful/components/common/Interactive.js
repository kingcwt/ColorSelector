...
const getRelativePosition = (node, event, touchId) => {
  ...
  return {
    left: clamp.clamp((pointer.pageX - (rect.left + getParentWindow(node).pageXOffset)) / rect.width),
    top: clamp.clamp((pointer.pageY - (rect.top + getParentWindow(node).pageYOffset)) / rect.height)
  };
};
...
const Interactive = /* @__PURE__ */ vue.defineComponent({
  name: "Interactive",
  props: {
    onMove: {
      type: Function
